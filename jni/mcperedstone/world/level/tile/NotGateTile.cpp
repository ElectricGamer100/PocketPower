...
bool NotGateTile::mayPlace(TileSource* region, int x, int y, int z, signed char side) {
	switch(side) {
	case 0: return false;
	case 1: return Tile::solid[region->getTile(x, y - 1, z).id];
	case 2: return Tile::solid[region->getTile(x, y, z + 1).id];
	case 3: return Tile::solid[region->getTile(x, y, z - 1).id];
	case 4: return Tile::solid[region->getTile(x + 1, y, z).id];
	case 5: return Tile::solid[region->getTile(x - 1, y, z).id];
	}
}
...
bool NotGateTile::canSurvive(TileSource* region, int x, int y, int z) {
	...
	switch(data) {
		case 5: return Tile::solid[region->getTile(x, y - 1, z).id];
		case 3: return Tile::solid[region->getTile(x, y, z - 1).id];
		case 4: return Tile::solid[region->getTile(x, y, z + 1).id];
		case 1: return Tile::solid[region->getTile(x - 1, y, z).id];
		case 2: return Tile::solid[region->getTile(x + 1, y, z).id];
	}
	return true;
}
